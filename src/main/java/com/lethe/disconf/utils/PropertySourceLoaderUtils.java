...
public class PropertySourceLoaderUtils {
    ...
    public static void loadProperty(List<String> confList, ConfigurableEnvironment environment) {
        for (String conf : confList) {
            String fileExtension = getFileExtension(conf);
            for (PropertySourceLoader propertySourceLoader : propertySourceLoaders) {
                if (canLoadFileExtension(propertySourceLoader, fileExtension)) {
    ...
    private static String getFileExtension(String conf) {
        int index = conf.lastIndexOf('.');
        return (index == -1) ? "" : conf.substring(index + 1);
    }
